                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div className="hidden md:block md:col-span-1">
                        ...
                        <form onSubmit={absenceForm.handleSubmit(onAddAbsence)} className="space-y-4 bg-background p-4 rounded-lg border border-border">
                            <div> <label htmlFor="date" className="block text-sm font-medium text-foreground">Data *</label> <Controller name="date" control={absenceForm.control} render={({ field }) => (<input type="date" {...field} className="input"/>)}/> </div>
                            <div> <label htmlFor="reason" className="block text-sm font-medium text-foreground">Motivo (opcional)</label> <Controller name="reason" control={absenceForm.control} render={({ field }) => (<textarea {...field} rows={3} className="input" placeholder="Ex: Consulta médica"/>)}/> </div>
                    ...
                    <div className="md:col-span-2">
                        ...
                        {absences.length === 0 ? (<p className="text-center py-10 text-muted-foreground bg-background rounded-lg">Nenhuma falta registrada.</p>) : (<ul className="space-y-3 max-h-96 overflow-y-auto pr-2"> {absences.map(absence => (<li key={absence.id} className="bg-card p-3 rounded-md border border-border flex justify-between items-center"> <div> <p className="font-medium">{new Date(absence.date + 'T00:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' })}</p> <p className="text-sm text-muted-foreground italic">{absence.reason || 'Sem motivo especificado'}</p> </div> <button onClick={() => { setAbsenceToDelete(absence); setIsDeleteAbsenceModalOpen(true); }} className="text-destructive hover:text-destructive p-1 rounded-full hover:bg-destructive/10"> <Trash2 className="w-4 h-4"/> </button> </li>))} </ul>)}
                    </div>
                </div>
            );
        case 'financial': 
            ...
            return <div className="max-w-2xl mx-auto"> <h4 className="text-lg font-medium text-foreground mb-4"> Resumo Financeiro - {moment().format('MMMM [de] YYYY')} </h4> <div className="bg-card border border-border rounded-lg shadow-sm divide-y"> <div className="p-4 flex justify-between items-center"> <p className="text-muted-foreground">Salário Base</p> <p className="font-semibold text-foreground">{formatCurrency(salary)}</p> </div> <div className="p-4 flex justify-between items-center"> <p className="text-muted-foreground">Taxa de Comissão</p> <p className="font-semibold text-foreground">{(professional.commission_rate || 0) * 100}%</p> </div> <div className="p-4 flex justify-between items-center bg-background"> <p className="text-muted-foreground"> Ganhos com Comissão ({financialSummary?.totalAppointments} atendimentos) </p> <p className="font-semibold text-success">{formatCurrency(commission)}</p> </div> <div className="p-4 flex justify-between items-center text-lg"> <p className="font-bold text-foreground">Ganhos Totais no Mês</p> <p className="font-bold text-primary">{formatCurrency(totalEarnings)}</p> </div> </div> </div>;
        case 'schedule': 
                ...
                <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        <div className="md:col-span-2">
                            ...
                            {exceptions.length === 0 ? (<p className="text-center py-10 text-muted-foreground bg-background rounded-lg">Nenhuma exceção registrada.</p>) : (<ul className="space-y-3 max-h-72 overflow-y-auto pr-2"> {exceptions.map(exception => (<li key={exception.id} className="bg-card p-3 rounded-md border border-border flex justify-between items-center"> <div> <p className="font-medium">{exception.description}</p> <p className="text-sm text-muted-foreground">{moment(exception.start_date).format('DD/MM/YY')} - {moment(exception.end_date).format('DD/MM/YY')}</p> </div> <button onClick={() => { setExceptionToDelete(exception); setIsDeleteExceptionModalOpen(true); }} className="text-destructive hover:text-destructive p-1 rounded-full hover:bg-destructive/10"> <Trash2 className="w-4 h-4"/> </button> </li>))} </ul>)}
                        </div>
                    </div>
                </div>
            );
        ...
        {(activeTab === 'absences' || activeTab === 'schedule') && (
            <div className="lg:hidden fixed bottom-6 right-6 z-40">
                <button
                    ...
                    className="bg-gradient-to-r from-primary to-secondary text-white rounded-full p-4 shadow-lg hover:scale-110 active:scale-100 transition-transform duration-200"
        ...
        {isAddModalOpen && (
            <div className="fixed inset-0 z-50 overflow-y-auto">
                <div className="flex items-center justify-center min-h-screen p-4 text-center">
                    ...
                    <div className="inline-block bg-card rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full">
                            ...
                            <form onSubmit={absenceForm.handleSubmit(onAddAbsence)}>
                                <div className="bg-card px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                  <div className="flex items-center justify-between mb-4">
                                      ...
                                      <button type="button" onClick={closeAddModal} className="text-gray-400 hover:text-muted-foreground"><X className="w-6 h-6" /></button>
                                  </div>
                                   <div className="space-y-4">
                                ...
                                <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                                    ...
                                    <button type="button" onClick={closeAddModal} className="mt-3 sm:mt-0 w-full sm:w-auto inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-card text-base font-medium text-gray-700 hover:bg-gray-50 sm:mr-3">Cancelar</button>
                                </div>
                            </form>
                        )}
                ...
                {data.map((item: any, index: number) => {
                    ...
                    return (
                        <li key={index} className={`${bgColors[index % 3]} rounded-lg p-3`}>
                            ...
                            <div className="bg-card rounded-full h-2 shadow-inner">
                                <div 
