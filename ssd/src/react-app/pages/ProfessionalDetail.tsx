            ...
            return (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div className="md:col-span-2">
                        ...
                        {absences.length === 0 ? (<p className="text-center py-10 text-gray-500 bg-gray-50 rounded-lg">Nenhuma falta registrada.</p>) : (<ul className="space-y-3 max-h-96 overflow-y-auto pr-2"> {absences.map(absence => (<li key={absence.id} className="bg-card p-3 rounded-md border flex justify-between items-center"> <div> <p className="font-medium">{new Date(absence.date + 'T00:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' })}</p> <p className="text-sm text-muted-foreground italic">{absence.reason || 'Sem motivo especificado'}</p> </div> <button onClick={() => { setAbsenceToDelete(absence); setIsDeleteAbsenceModalOpen(true); }} className="text-red-500 hover:text-red-700 p-1 rounded-full hover:bg-red-50"> <Trash2 className="w-4 h-4"/> </button> </li>))} </ul>)}
                    </div>
                </div>
            );
        case 'financial': 
                ...
                <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        <div className="md:col-span-2">
                            ...
                            {exceptions.length === 0 ? (<p className="text-center py-10 text-gray-500 bg-gray-50 rounded-lg">Nenhuma exceção registrada.</p>) : (<ul className="space-y-3 max-h-72 overflow-y-auto pr-2"> {exceptions.map(exception => (<li key={exception.id} className="bg-card p-3 rounded-md border flex justify-between items-center"> <div> <p className="font-medium">{exception.description}</p> <p className="text-sm text-muted-foreground">{moment(exception.start_date).format('DD/MM/YY')} - {moment(exception.end_date).format('DD/MM/YY')}</p> </div> <button onClick={() => { setExceptionToDelete(exception); setIsDeleteExceptionModalOpen(true); }} className="text-red-500 hover:text-red-700 p-1 rounded-full hover:bg-red-50"> <Trash2 className="w-4 h-4"/> </button> </li>))} </ul>)}
                        </div>
                    </div>
                </div>
            );
