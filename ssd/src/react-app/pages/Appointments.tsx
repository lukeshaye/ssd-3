        ...
        <div className="sm:flex sm:items-center sm:justify-between">
          ...
          <div className="mt-4 sm:mt-0 flex items-center space-x-3">
            ...
            <button
              ...
              className="hidden sm:inline-flex items-center justify-center rounded-md border border-transparent bg-gradient-to-r from-primary to-secondary px-4 py-2 text-sm font-medium text-white shadow-sm hover:from-pink-600 hover:to-violet-600"
            >
        ...
        <div className="mt-8">
          <div className="lg:col-span-12">
            <div className="bg-card rounded-xl shadow-sm border border-border min-h-[60vh] overflow-hidden">
              {/* Header melhorado */}
              <div className="bg-gradient-to-r from-pink-50 to-violet-50 p-6 border-b border-border">
                <div className="flex justify-between items-center">
                  <button 
                    onClick={() => handleDayNavigation('prev')} 
                    className="p-3 rounded-full bg-card hover:bg-accent transition-all duration-200 shadow-sm hover:shadow-md"
                  >
              ...
              <div className="p-4 sm:p-6">
                {Object.keys(groupedAppointments).length === 0 ? (
                  <div className="text-center py-20">
                    ...
                    <button
                      onClick={() => handleOpenModal()}
                      className="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-lg text-white bg-gradient-to-r from-primary to-secondary hover:from-pink-600 hover:to-violet-600 shadow-lg hover:shadow-xl transition-all duration-200"
                    >
                    ...
                    {Object.entries(groupedAppointments).map(([time, apps]) => (
                      <div key={time} className="relative flex gap-x-3">
                        <div className="flex-shrink-0 flex flex-col items-center">
                          <div className="bg-gradient-to-r from-primary to-secondary text-white rounded-lg px-3 py-1 mb-2">
                          ...
                          <div className="relative flex h-full w-6 justify-center items-center">
                            ...
                            <div className="absolute top-0 w-5 h-5 rounded-full bg-gradient-to-r from-primary to-secondary border-2 border-white shadow-sm"></div>
                          </div>
                        </div>
                        <div className="flex-grow pb-6">
                          <div className="space-y-3">
                            {apps.map(app => {
                                ...
                                <div 
                                    key={app.id} 
                                    className="bg-card p-4 rounded-xl border border-border cursor-pointer hover:shadow-md hover:border-pink-200 transition-all duration-200 group"
                                ...
                                >
                                    <div className="flex justify-between items-start">
                                        <div className="flex items-start space-x-3">
                                          ...
                                          />
                                          <div>
                                            <p className="font-semibold text-foreground group-hover:text-pink-600 transition-colors">
                                              {service?.name || 'Serviço não encontrado'}
                                            </p>
                                            <p className="text-sm text-muted-foreground mt-1">{client?.name || 'Cliente não encontrado'}</p>
                                            <div className="flex items-center mt-2">
                                              ...
                                              <p className="text-xs text-muted-foreground">{professional?.name || 'Profissional não encontrado'}</p>
                                            </div>
                                          </div>
                                        </div>
                                        <div className="text-right flex-shrink-0 ml-4">
                                          <div className="bg-gray-50 rounded-lg px-3 py-1">
                                            <p className="text-sm font-medium text-foreground">{moment(app.end_date).diff(moment(app.appointment_date), 'minutes')} min</p>
                                          </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                   <div className="bg-card px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                     <div className="flex items-center justify-between mb-4">
                       <h3 className="text-lg font-medium text-foreground">{editingAppointment ? 'Editar Agendamento' : 'Novo Agendamento'}</h3>
                     ...
                     <div className="space-y-4">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className="flex-shrink-0 rounded-full bg-gradient-to-r from-primary to-secondary p-2 text-white shadow-sm hover:from-pink-600 hover:to-violet-600 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-offset-2"
                        ...
                        <div>
                          <label htmlFor="price" className="block text-sm font-medium text-foreground">Preço (R$) *</label>
                          <Controller
                              name="price"
                              control={control}
                       ...
                       <div>
                          <label htmlFor="appointment_date_date" className="block text-sm font-medium text-foreground mb-1">Data *</label>
                          <Controller
                   ...
                   <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse items-center">
                      <button type="submit" className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-gradient-to-r from-primary to-secondary text-base font-medium text-white hover:from-pink-600 hover:to-violet-600 sm:ml-3 sm:w-auto sm:text-sm">
                       {editingAppointment ? 'Atualizar' : 'Criar'}
                     </button>
                     <button type="button" onClick={handleCloseModal} className="mt-3 w-full inline-flex justify-center rounded-md border border-border shadow-sm px-4 py-2 bg-card text-base font-medium text-foreground hover:bg-accent sm:mt-0 sm:w-auto sm:text-sm">
                       Cancelar
                     </button>
                      {editingAppointment && (
                         <button
