      ...
      <div className="px-4 sm:px-6 lg:px-8 pb-24 lg:pb-8">
        <div className="sm:flex sm:items-center sm:justify-between">
          ...
          <div className="mt-4 sm:mt-0 sm:ml-16 flex items-center space-x-3">
            ...
            <button type="button" onClick={() => setIsModalOpen(true)} className="hidden sm:inline-flex items-center justify-center rounded-md border border-transparent bg-gradient-to-r from-primary to-secondary px-4 py-2 text-sm font-medium text-primary-foreground shadow-sm hover:from-primary/90 hover:to-secondary/90">
              <Plus className="w-4 h-4 mr-2" /> Nova Entrada
            </button>
          </div>
        </div>
        
        {error && !isModalOpen && <div className="bg-destructive/10 p-4 rounded-md my-4 flex items-center"><AlertCircle className="h-5 w-5 text-destructive mr-3" /><p className="text-sm text-destructive">{error}</p></div>}
              ...
              <div className="flex items-center">
                ...
                <div className="ml-4 w-0 flex-1">
                  <dl>
                    ...
                    <dd className="text-2xl font-bold text-foreground">{formatCurrency(kpis.monthlyRevenue)}</dd>
                  </dl>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center">
                ...
                <div className="ml-4 w-0 flex-1">
                  <dl>
                    ...
                    <dd className="text-2xl font-bold text-foreground">{formatCurrency(kpis.monthlyExpenses)}</dd>
                  </dl>
                </div>
              </div>
            </div>
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-end gap-4 flex-wrap">
                      ...
                      <div className="flex items-center justify-center">
                        <button onClick={handlePreviousMonth} className="p-2 rounded-full hover:bg-accent transition-colors flex-shrink-0">
                        ...
                        <button onClick={handleNextMonth} className="p-2 rounded-full hover:bg-accent transition-colors flex-shrink-0">
                  ...
                  <div className="text-center py-16">
                    ...
                    {(typeFilter === 'all' && frequencyFilter === 'all') && (
                      <button
                        ...
                        className="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-lg text-white bg-gradient-to-r from-primary to-secondary hover:from-primary/90 hover:to-secondary/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500 transition-colors"
                      >
                    ...
                    <>
                        <div className="lg:hidden p-4 space-y-4">
                            {filteredEntries.map((entry) => (
                                <div key={entry.id} className="bg-card p-5 rounded-xl shadow-sm border border-border hover:shadow-md transition-shadow">
                                    <div className="flex justify-between items-start gap-3 mb-4">
                                      ...
                                      <div className="flex items-center justify-end gap-2 pt-3 border-t border-gray-100">
                                        <button 
                                          ...
                                          className="inline-flex items-center px-3 py-1.5 text-xs font-medium rounded-lg text-foreground bg-muted hover:bg-gray-200 transition-colors"
                                        >
                        ...
                        <div className="hidden lg:block overflow-x-auto">
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gradient-to-r from-pink-50 to-violet-50">
                                  <tr>
                                    <th className="px-6 py-4 text-left text-xs font-semibold text-foreground uppercase tracking-wider">Data</th>
                                    <th className="px-6 py-4 text-left text-xs font-semibold text-foreground uppercase tracking-wider">Descrição</th>
                                    <th className="px-6 py-4 text-left text-xs font-semibold text-foreground uppercase tracking-wider">Tipo</th>
                                    <th className="px-6 py-4 text-left text-xs font-semibold text-foreground uppercase tracking-wider">Frequência</th>
                                    <th className="px-6 py-4 text-left text-xs font-semibold text-foreground uppercase tracking-wider">Valor</th>
                                    <th className="px-6 py-4 text-right text-xs font-semibold text-foreground uppercase tracking-wider">Ações</th>
                                  </tr>
                                </thead>
                                <tbody className="bg-card divide-y divide-gray-100">
                                    {filteredEntries.map((entry) => (
                                        <tr key={entry.id} className="hover:bg-accent transition-colors">
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-foreground">{formatDate(entry.entry_date)}</td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                              <div className="flex items-center justify-end gap-2">
                                                <button 
                                                  onClick={() => handleEditEntry(entry)} 
                                                  className="inline-flex items-center px-2 py-1 text-xs font-medium rounded-lg text-foreground bg-muted hover:bg-gray-200 transition-colors"
                                                >
        ...
        <div className="relative flex flex-col items-end">
            ...
            >
                <div className="flex items-center gap-x-3">
                    <span className="bg-card text-foreground text-sm font-semibold px-3 py-1.5 rounded-md shadow-sm">
                        Nova Entrada
                    </span>
                    <button
                        onClick={() => { setIsModalOpen(true); setIsFabMenuOpen(false); }}
                        className="bg-card text-foreground rounded-full p-3 shadow-lg hover:bg-accent transition-colors"
            ...
            <button 
                ...
                className="relative z-10 bg-gradient-to-r from-primary to-secondary text-white rounded-full p-4 shadow-lg hover:scale-110 active:scale-100 transition-all duration-300"
              ...
              <form onSubmit={handleSubmit(onSubmit as any)}>
                <div className="bg-card px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button type="button" onClick={handleCloseModal} className="text-gray-400 hover:text-muted-foreground"><X className="w-6 h-6" /></button>
                  </div>
                  {error && (
                ...
                <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                  <button type="submit" disabled={isSubmitting} className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-gradient-to-r from-primary to-secondary text-base font-medium text-white hover:from-primary/90 hover:to-secondary/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500 sm:ml-3 sm:w-auto sm:text-sm disabled:opacity-50">
                    {isSubmitting ? 'Salvando...' : (editingEntry ? 'Atualizar' : 'Criar')}
                  </button>
                  <button type="button" onClick={handleCloseModal} className="mt-3 w-full inline-flex justify-center rounded-md border border-border shadow-sm px-4 py-2 bg-card text-base font-medium text-foreground hover:bg-accent focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                    Cancelar
                  </button>
                </div>
              </form>
